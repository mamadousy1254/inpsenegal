...
export function BulletinsScientifiquesClient({
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="py-6 bg-gray-50/80 border-y border-green-100">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            ...
            <p className="text-sm text-gray-500">
              <span className="font-semibold text-green-800">{filtered.length}</span> bulletin
              {filtered.length > 1 ? "s" : ""}
            </p>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="p-8 space-y-5">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold px-4 py-1.5 rounded-full bg-gradient-to-r from-green-100 to-green-50 text-green-800 border border-green-200">
                        {bulletin.issue ?? `Vol. ${bulletin.year}`}
                      </span>
                      <span className="text-sm font-semibold flex items-center gap-1.5 bg-gradient-to-r from-[#0f3d2e] to-[#8b5e3c] bg-clip-text text-transparent">
                        <Calendar size={14} className="text-green-700" />
                        {bulletin.year}
                      </span>
                    </div>
                    <div className="flex items-start gap-3">
                      <BookMarked className="h-5 w-5 text-green-700 mt-0.5 shrink-0" />
