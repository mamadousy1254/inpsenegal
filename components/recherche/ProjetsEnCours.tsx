...
const STATUS_STYLE: Record<ResearchProjectStatus, { bg: string; text: string }> = {
  en_cours: { bg: "bg-amber-100", text: "text-amber-800" },
  termine: { bg: "bg-gray-100", text: "text-gray-700" },
  partenariat: { bg: "bg-sky-100", text: "text-sky-800" },
};

const FALLBACK_PROJECTS: SerializedResearchProject[] = [
  {
    _id: "fallback-casamance",
...
export function ProjetsEnCours({
  ...
  projects?: SerializedResearchProject[];
}) {
  const items = projects && projects.length > 0 ? projects : FALLBACK_PROJECTS;
  const [selected, setSelected] = useState<SerializedResearchProject | null>(null);

  const selectedStyle = selected
    ? STATUS_STYLE[selected.projectStatus] ?? STATUS_STYLE.en_cours
    : null;
    ...
    >
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="mt-14 space-y-5">
          {items.map((project, i) => {
            const st = STATUS_STYLE[project.projectStatus] ?? STATUS_STYLE.en_cours;
            return (
              <motion.article
                key={project._id}
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      >
                        {RESEARCH_PROJECT_STATUS_LABELS[project.projectStatus]}
                      </span>
          ...
          {selected && (
            <>
              <DialogHeader>
                <div className="flex flex-wrap items-center gap-2.5">
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${selectedStyle?.bg} ${selectedStyle?.text}`}
                  >
                    {RESEARCH_PROJECT_STATUS_LABELS[selected.projectStatus]}
                  </span>
                  <span className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Calendar className="h-3 w-3" aria-hidden />
                    {selected.year}
                ...
                <DialogTitle className="mt-3 text-left text-xl leading-snug">
                  {selected.title}
                </DialogTitle>
              </DialogHeader>
