...
export async function PATCH(req: Request, context: RouteContext) {
  ...
  await connectDB();
  const item = await CartothequeItemModel.findById(id);
  if (!item) {
  ...
  if (data.legende !== undefined) item.legende = data.legende;
  ...
  return NextResponse.json(serializeCartothequeItem(item));
}
